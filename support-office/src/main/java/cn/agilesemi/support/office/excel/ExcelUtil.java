...
public class ExcelUtil {
    ...
    public static void cloneRows(HSSFSheet sheet, int sourceStartRowIndex, int rowsNum,
        ...
        for (int i = 0; i < sheet.getNumMergedRegions(); i++) {
            ...
            if (address.getFirstRow() == source.getRowNum()) {
                CellRangeAddress newAddress = new CellRangeAddress(target.getRowNum(),
                        target.getRowNum() + address.getLastRow() - address.getFirstRow(),
    ...
    public static void mergeCells(HSSFSheet sheet, int firstRowIndex, int firstColumnIndex,
            int lastRowIndex, int lastColumnIndex) {
        sheet.addMergedRegion(new CellRangeAddress(firstRowIndex, lastRowIndex, firstColumnIndex,
                lastColumnIndex));
    }
