...
public class AliyunSmsContentSender implements SmsContentSender {
	...
	@Override
	public SmsSendResult send(final String content, final int maxCount, final String... mobilePhones) {
	...
	private SmsSendResult send(final String content, final String... mobilePhones) {
		...
		try {
			...
			DefaultProfile.addEndpoint(regionId, regionId, "Sms", "sms.aliyuncs.com");
	...
	private class SendCommand implements Runnable {
		...
		public SendCommand(final String content, final String[] mobilePhones, final SmsSendCallback callback) {
