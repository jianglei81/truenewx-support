...
public class HttpSmsContentSender extends AbstractSmsContentSender {
    ...
    private Set<String> send(List<String> contents, int index, String... mobilePhones)
        ...
        HttpRequestMethod method = EnumUtils.getEnum(HttpRequestMethod.class,
                this.strategy.getRequestMethod());
        if (method == null) {
            method = HttpRequestMethod.GET;
        }
        Binate<Integer, String> binate = HttpClientUtil.request(this.strategy.getUrl(), params,
                method, this.strategy.getEncoding());
